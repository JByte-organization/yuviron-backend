...
ALTER TABLE `artist_team_members` DROP INDEX `IX_artist_team_members_ArtistId_Role`;

ALTER TABLE `outbox_messages` ADD `NextAttemptUtc` datetime(6) NULL;

ALTER TABLE `outbox_messages` ADD `RetryCount` int NOT NULL DEFAULT 0;

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20260307210346_AddOutboxRetryLogic', '9.0.0');

ALTER TABLE `outbox_messages` DROP INDEX `IX_outbox_messages_ProcessedOnUtc_OccurredOnUtc`;

ALTER TABLE `artist_team_members` ADD `OwnerUniqueConstraint` int AS (CASE WHEN Role = 1 THEN 1 ELSE NULL END) STORED NULL;


                UPDATE artist_team_members atm
                JOIN (
                    SELECT ArtistId, MIN(Id) as MinId
                    FROM artist_team_members
                    WHERE Role = 1
                    GROUP BY ArtistId
                    HAVING COUNT(*) > 1
                ) as duplicates ON atm.ArtistId = duplicates.ArtistId AND atm.Id <> duplicates.MinId
                SET atm.Role = 2
                WHERE atm.Role = 1;
            

CREATE INDEX `IX_outbox_messages_ProcessedOnUtc_NextAttemptUtc_OccurredOnUtc` ON `outbox_messages` (`ProcessedOnUtc`, `NextAttemptUtc`, `OccurredOnUtc`);

CREATE UNIQUE INDEX `IX_artist_team_members_ArtistId_OwnerUniqueConstraint` ON `artist_team_members` (`ArtistId`, `OwnerUniqueConstraint`);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20260307213018_FixOutboxAndArtistIndexes', '9.0.0');
