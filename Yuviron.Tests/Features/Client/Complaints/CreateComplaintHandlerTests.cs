...
public class CreateComplaintHandlerTests
{
    ...
    public async Task Handle_Should_Create_Complaint_And_Increment_Counter()
    {
        ...
        dbContext.Add(User.Create("user@mail.com", "hash", "User", true, true, utcNow));
        await dbContext.SaveChangesAsync();

        var handler = new CreateComplaintHandler(dbContext, dbContext, dbContext, dbContext, currentUserMock.Object, TimeProvider.System);
        ...
        var complaint = await dbContext.Set<Complaint>().FindAsync(complaintId);
    ...
    public async Task Handle_Should_Create_Complaint_For_User_Target()
    {
        ...
        var utcNow = DateTime.UtcNow;
        dbContext.Add(User.Create("reporter@mail.com", "hash", "Reporter", true, true, utcNow));
        var targetUser = User.Create("target@mail.com", "hash", "Target", true, true, utcNow);
        dbContext.Add(targetUser);
        dbContext.Add(UserProfile.Create(targetUser.Id, "Target", null, null, null, null, null, utcNow, Gender.Male, utcNow));
        await dbContext.SaveChangesAsync();
