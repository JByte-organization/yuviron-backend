...
public sealed class StreamTokenService : IStreamTokenService
{
    ...
    public string GenerateToken(Guid trackId, int quality, DateTimeOffset expiration, Guid userId, string ipAddress)
    {
        var expirationUnix = expiration.ToUnixTimeSeconds();

        var payload = $"{trackId:N}:{quality}:{expirationUnix}:{userId:N}:{ipAddress}";
    ...
    public bool ValidateToken(Guid trackId, int quality, long expirationUnix, Guid userId, string ipAddress, string token)
    {
